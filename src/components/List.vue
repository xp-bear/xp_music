      ...
      <el-card v-show="show" class="box-card transition-box">
        <el-table :data="songs" stripe style="width: 100%">
          ...
          <el-table-column label="歌手" width="180">
            <template slot-scope="scope">
              <div @click="toSingerPage(scope.row.artists[0].name)" class="at-singer">{{ scope.row.artists[0].name }}</div>
            </template>
          </el-table-column>
          <el-table-column label="操作">
            <template slot-scope="scope">
              <div>
                ...
                <i v-show="scope.row.mvid !== 0" class="el-icon-video-camera-solid" title="播放MV" @click="toMV(scope.row.mvid, scope.row.name)"></i>
                <!-- </el-tooltip> -->
              </div>
            </template>
          </el-table-column>
